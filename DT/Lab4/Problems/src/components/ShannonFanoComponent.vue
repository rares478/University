<template>
  <div class="shannon-fano-container">
    <h1>Shannon-Fano Compression</h1>
    ...
    <div id="shannon-fano-tree-container" class="tree-container"></div>
  </div>
</template>

<script>
import { encode, getCompressionStats } from '../ShannonFano.js';
...
export default {
  ...
  methods: {
    compressText() {
      ...
      this.shannonFanoTree = this.compressionResult.tree;
      ...
      // Render the tree
      this.$nextTick(() => {
        this.renderShannonFanoTree();
      });
    },

    renderShannonFanoTree() {
      if (!this.shannonFanoTree) return;

      const container = d3.select('#shannon-fano-tree-container');
      container.selectAll("*").remove();

      // Convert Shannon-Fano tree to hierarchical format for D3
      const hierarchyData = this.convertToHierarchy(this.shannonFanoTree);
